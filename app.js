...
function bind(){
 ...
 $('btnLimpar').onclick=()=>{if(confirm('Apagar todos os lançamentos deste aparelho?')){state.lancamentos=[];save();renderAll();}};
...
function delLanc(id){ if(confirm('Excluir este lançamento?')){state.lancamentos=state.lancamentos.filter(x=>x.id!==id);save();renderAll();}}
